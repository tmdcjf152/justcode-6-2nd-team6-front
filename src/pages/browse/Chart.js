  ...
  const onClickhandle = () => {
    fetch("http://13.125.174.118:8000/play/addsongs/popular/1", {
      ...
      .then((plData) => {
        ...
        const musicTracksId = musicTracks.map((el, i) => String(el.songId));
        const filteredSelectedPlData = selectedPlData.filter(
          (el) => musicTracksId.includes(String(el.songId)) === false
        );
        setMusicTracks([...filteredSelectedPlData, ...musicTracks]);
        setAlertOn("재생목록에 추가되었습니다. 중복된 곡은 제외됩니다.");
        setCheckList([]);
      });
  };
    ...
    <Fade>
      <StyledTrack>
        <div className="detail-track-inner-box">
            ...
            <table className="detail-track-list-table">
                ...
                <tr>
                  <th scope="col">
                    <input
                      ...
                      checked={chart.length === checkList.length ? true : false}
                      onClick={() => {
                        if (checkList.length < chart.length) {
                          setCheckList(chart.map((el) => el.songId));
                        } else setCheckList([]);
                      }}
                    />
                  </th>
              ...
              <tbody>
                {chart.map((data) => {
                  const songPlay = () => {
                    fetch(
                      `http://3.34.53.252:8000/play/addsongs/popular/${params.id} `,
                      {
                        headers: {
                          Authorization: sessionStorage.getItem("token"),
                        },
                  ...
                  {
                    chart &&
                      chart.slice(0, visible).map((song, index) => {
                        return (
                          <tr key={song.songId}>
                            <td
                              ...
                              onClick={() => {
                                if (isSelectClicked === true)
                                  onCheckedElement(
                                    checkList.includes(song.songId),
                                    song.songId
                                  );
                              }}
                            >
                            ...
                            <td className="detail-track-list-info-wrap">
                              <div className="detail-track-list-info-box">
                                <div className="detail-track-list-info-thumb">
                                  <Link
                                    to={`/detail/album/${song.albumId}/details`}
                            ...
                            <td className="detail-track-list-artist-box">
                              <Link
                                to={`/detail/artist/${song.atsId}/songs`}
          ...
          {/* 모달 창 */}
          {!isSelectClicked || checkList.length === 0 || (
              ...
              <div className="edit-container">
                ...
                <div className="edit-box">
                  <div
                    ...
                    onClick={() => {
                      fetch(
                        "http://3.34.53.252:8000/play/addsongs/popular/1 ",
                        {
                          headers: {
                            Authorization: sessionStorage.getItem("token"),
                          },
                        ...
                        .then((plData) => {
                          ...
                          const selectedPlData = plData.filter(
                            (el, i) => setCheckList.includes(el.songId) === true
                          );
